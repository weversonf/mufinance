...
## Nova solicitação: refinamento visual do Extrato

- [x] Adicionar separadores sutis e consistentes entre grupos de dias no Extrato.
- [x] Reduzir títulos, datas, descrições, categorias e valores para a escala tipográfica das demais telas.
- [x] Validar a leitura em desktop/mobile, tema claro/escuro, check/build e registrar checkpoint.

## Nova solicitação: simplificação dos grupos diários

- [x] Remover do cabeçalho diário os totais de entradas e saídas.
- [x] Centralizar o nome da data no cabeçalho de cada dia.
- [x] Deixar somente o saldo diário no fechamento, sem a indicação adicional de receitas ou despesas.
- [x] Validar TypeScript, build, acessibilidade, tema, desktop/mobile e registrar checkpoint.

## Nova solicitação: valores e ícones por categoria no Extrato

- [x] Padronizar o tamanho dos valores dos lançamentos sem depender de estilo inline isolado.
- [x] Exibir um ícone semântico de acordo com a categoria de cada lançamento.
- [x] Validar alinhamento, legibilidade, tema, desktop/mobile, check/build e registrar checkpoint.

## Nova solicitação: status Prevista ou Realizada

- [x] Adicionar o campo de status Prevista/Realizada ao modelo e aos dados demonstrativos.
- [x] Incluir o seletor de status no modal de criação e edição de transações.
- [x] Exibir o status no Extrato e permitir filtrar por Prevista ou Realizada.
- [x] Preservar o cálculo de saldo realizado e validar criação, edição, filtros, tema, desktop/mobile, check/build e checkpoint.

## Nova solicitação: transferência entre contas e chaves de opção

- [x] Adicionar o tipo Transação entre contas ao modal e ao modelo de lançamento.
- [x] Permitir selecionar conta de origem e conta de destino, impedindo que sejam iguais.
- [x] Converter as opções binárias adequadas do modal para chaves liga/desliga, preservando acessibilidade.
- [x] Atualizar saldos e Extrato para transferências sem classificá-las como receita ou despesa.
- [x] Validar os fluxos existentes, check/build, tema, desktop/mobile e registrar checkpoint.

## Nova solicitação: ícone dinâmico do Veículo

- [x] Usar o ícone de carro quando o perfil do veículo for Carro e o ícone de moto quando for Moto.
- [x] Sincronizar a escolha na sidebar, navegação mobile, tela dedicada e prévias do veículo.
- [x] Validar alteração do tipo, tema, desktop/mobile, check/build e registrar checkpoint.

## Nova solicitação: versão principal e publicação

- [ ] Verificar se a pasta `react/` permanece vinculada ao Firebase do projeto inicial.
- [ ] Confirmar que a versão React está preparada como principal no repositório, sem apagar o vínculo existente.
- [ ] Revisar a estratégia de publicação e orientar sobre Manus Hosting versus Vercel.

## Nova solicitação: React como página principal na raiz
