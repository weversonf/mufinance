...
import { accounts, budgets, cashflowData, creditCards as initialCreditCards, defaultFinanceCategories, formatBRL, formatCompactBRL, navItems, spendingData, transactions, upcomingBills, vehicleProfile as initialVehicleProfile, type Account, type CreditCard as CreditCardData, type FinanceCategory, type Transaction, type VehicleProfile } from "@/lib/financeData";
  ...
  return (
    <div className={`app-shell ${compactMode ? "app-shell--compact" : ""} ${hideValues ? "app-shell--values-hidden" : ""}`}>
      <AnimatePresence>{mobileOpen && <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mobile-scrim" aria-label="Fechar menu" onClick={() => setMobileOpen(false)} />}</AnimatePresence>
          <Sidebar profile={profile} collapsed={sidebarCollapsed} mobileOpen={mobileOpen} activeNav={activeNav} onSelect={handleNavSelect} onCollapse={() => setSidebarCollapsed((value) => !value)} onExport={exportTransactions} />
