...
import { accounts, budgets, cashflowData, creditCards as initialCreditCards, defaultFinanceCategories, formatBRL, formatCompactBRL, navItems, spendingData, transactions, upcomingBills, type CreditCard as CreditCardData, type FinanceCategory, type Transaction } from "@/lib/financeData";
...
export default function FinanceDashboard() {
  ...
  const submitTransfer = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setLocalTransactions((current) => [{ id: `transfer-${Date.now()}`, date: "13 ago", dateISO: "2026-08-13", payee: `Transferência para ${transferDestination}`, category: "Transferências", account: "Conta •7045", sourceType: "account", sourceId: "Conta •7045", type: "expense", amount }, ...current]);
    setTransferAmount("");
    closePanels();
  ...
  const submitDeposit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setLocalTransactions((current) => [{ id: `deposit-${Date.now()}`, date: "13 ago", dateISO: "2026-08-13", payee: "Depósito PIX", category: "Receitas", account: "Conta •7045", sourceType: "account", sourceId: "Conta •7045", type: "income", amount }, ...current]);
    setDepositAmount("");
    closePanels();
    ...
    <div className={`app-shell ${compactMode ? "app-shell--compact" : ""} ${hideValues ? "app-shell--values-hidden" : ""}`}>
      ...
      <main className={`main-area ${sidebarCollapsed ? "main-area--wide" : ""}`}>
        ...
        <motion.div className="dashboard-content" initial="hidden" animate="visible" variants={pageVariants}>
          {currentView === "statement" && <StatementPage transactions={filteredTransactions} periodLabel={selectedPeriod} onBack={() => { setCurrentView("dashboard"); setActiveNav("Início"); }} onEdit={editTransaction} onFilter={() => setActionPanel("filters")} onExport={exportTransactions} />}
          {currentView === "vehicle" && <VehiclePage onBack={() => { setCurrentView("dashboard"); setActiveNav("Início"); }} onAction={handleVehicleAction} />}
...
function AccountsPanelContent({ account, selectedAccount, accountConnected, reportTransactions, onSelectAccount }: { account?: (typeof accounts)[number]; selectedAccount: string | null; accountConnected: boolean; reportTransactions: Transaction[]; onSelectAccount: (name: string) => void }) {
  const accountDigits = account?.number.replace(/\D/g, "");
  const accountTransactions = accountDigits ? reportTransactions.filter((item) => item.account.replace(/\D/g, "").includes(accountDigits)) : [];
  ...
  return <div className="modal-rich-content accounts-panel"><div className="modal-summary-strip"><div><span>Carteiras</span><strong>{accounts.length - 1}</strong></div><div><span>Patrimônio local</span><strong>R$ 32,4K</strong></div><div><span>Movimentações</span><strong>{accountTransactions.length}</strong></div></div>{account ? <div className="account-detail-card"><div className={`account-detail-icon account-icon--${account.tone}`}>{navIcon(account.icon)}</div><div><p className="eyebrow">CONTA SELECIONADA</p><h3>{account.name}</h3><span>{account.number} · {account.change}</span></div><strong>{account.value}</strong></div> : <div className="modal-empty-note"><Wallet size={17} /><span>Selecione uma carteira abaixo para ver seus detalhes.</span></div>} {account && <div className="account-flow-strip"><div><span>Entradas no período</span><strong className="income-text">+{formatCompactBRL(inflow)}</strong></div><div><span>Saídas no período</span><strong className="expense-text">−{formatCompactBRL(outflow)}</strong></div></div>}<div className="modal-section-heading"><div><p className="eyebrow">CARTEIRAS LOCAIS</p><h3>Onde seu dinheiro está</h3></div><span className="modal-section-count">{accounts.length - 1} contas</span></div><div className="action-list action-list--rich">{accounts.filter((item) => item.icon !== "card").map((item) => <button key={item.name} className={`action-list-item ${selectedAccount === item.name ? "is-selected" : ""}`} onClick={() => onSelectAccount(item.name)}><span className={`account-icon account-icon--${item.tone}`}>{navIcon(item.icon)}</span><span className="action-list-copy"><strong>{item.name}</strong><small>{item.value} · {item.number}</small></span><span className="status-pill">{item.change}</span><ChevronRight size={15} /></button>)}</div>{accountConnected && <p className="dialog-hint"><Check size={14} /> Conta demonstrativa conectada nesta sessão.</p>}</div>;
}
