...
const navAnchors: Record<string, string> = { Início: "overview", Extrato: "transactions", Relatórios: "spending", Cartões: "balance", Metas: "budget", Orçamento: "budget" };
const searchItems: SearchItem[] = [
  ...
  { label: "Sua carteira", nav: "Cartões", anchor: "balance" },
    ...
    <div className={`app-shell ${compactMode ? "app-shell--compact" : ""}`}>
      ...
      <main className={`main-area ${sidebarCollapsed ? "main-area--wide" : ""}`}>
        ...
        <motion.div className="dashboard-content" initial="hidden" animate="visible" variants={pageVariants}>
          ...
          <motion.section id="transactions" className="surface-card transactions-card" variants={child}><CardHeader eyebrow="ATIVIDADE RECENTE" title="Últimos lançamentos" subtitle="Movimentações recentes em suas contas" action={<div className="transactions-actions"><button className="soft-button soft-button--small" onClick={() => setActionPanel("filters")}><Filter size={14} /> Filtrar</button><button className="text-button" onClick={() => { setActiveNav("Extrato"); scrollTo("transactions"); action("Extrato completo aberto"); }}>Ver tudo <ChevronRight size={14} /></button></div>} /><div className="transactions-table-wrap"><table><thead><tr><th>Data</th><th>Descrição</th><th>Categoria</th><th>Conta</th><th className="amount-cell">Valor</th><th /></tr></thead><tbody>{filteredTransactions.map((transaction) => <tr key={`${transaction.date}-${transaction.payee}-${transaction.amount}`}><td className="muted-cell">{transaction.date}</td><td><span className={`transaction-icon transaction-icon--${transaction.type}`}>{transaction.type === "income" ? <ArrowDownLeft size={15} /> : <ArrowUpRight size={15} />}</span><strong>{transaction.payee}</strong></td><td><span className="category-chip">{transaction.category}</span></td><td className="muted-cell">{transaction.account}</td><td className={`amount-cell ${transaction.type === "income" ? "income-text" : "expense-text"}`}>{transaction.type === "income" ? "+" : "−"}{formatBRL(transaction.amount).replace("R$ ", "R$ ")}</td><td><button className="row-more" onClick={() => { setSelectedTransaction(transaction); setActionPanel("transaction"); }} aria-label={`Mais opções para ${transaction.payee}`}><MoreHorizontal size={16} /></button></td></tr>)}</tbody></table>{filteredTransactions.length === 0 && <div className="table-empty"><Filter size={18} /><strong>Nenhum lançamento encontrado</strong><span>Limpe os filtros para ver todas as movimentações.</span></div>}</div></motion.section>
