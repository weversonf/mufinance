...
import { InitialSetupScreen, type InitialSetupData } from "./InitialSetupScreen";
...
type ActionPanel = "transfer" | "deposit" | "pay-bills" | "schedule" | "report" | "accounts" | "account-new" | "account-edit" | "card-edit" | "budget" | "transaction" | "about" | "support" | "privacy" | "insights" | "settings" | "session" | null;
...
type NewAccountPayload = { name: string; balance: number; icon: string };
...
const kpiIcons = [
  { label: "Saldo total", icon: Wallet, tone: "mint" },
  { label: "Receitas do mês", icon: ArrowDownLeft, tone: "blue" },
  { label: "Despesas do mês", icon: ArrowUpRight, tone: "lavender", negative: true },
  { label: "Taxa de economia", icon: Sparkles, tone: "peach" },
];
...
const emptyVehicle: VehicleProfile = { type: "car", manufacturer: "", model: "", year: new Date().getFullYear(), fuel: "Gasolina", city: "" };
type UpcomingBill = { label: string; date: string; amount: string; days: string; tone: string };
...
export default function FinanceDashboard() {
  ...
  const chartData = useMemo(() => {
    ...
    return Array.from({ length: count }, (_, index) => {
      ...
      const prefix = `${date.getFullYear()}-${String(monthIndex + 1).padStart(2, "0")}`;
      const monthItems = localTransactions.filter((item) => item.dateISO?.startsWith(prefix));
      ...
      return { month, periodLabel: `${month} ${date.getFullYear()}`, income, expenses, net: income - expenses, isReference: date.getFullYear() === selectedYear && monthIndex === selectedMonth };
    });
  }, [localTransactions, selectedMonth, selectedYear, timeRange]);
  ...
  const spendingData = useMemo(() => {
    const colors = ["#81cfc1", "#a99bea", "#f4b37a", "#7bb9e7", "#e8eaf0"];
    const totals = periodTransactions.filter((item) => item.type === "expense").reduce<Record<string, number>>((result, item) => ({ ...result, [item.category]: (result[item.category] ?? 0) + item.amount }), {});
  ...
  const periodTotals = useMemo(() => {
    ...
    return { income, expenses, net: income - expenses, savingsRate: income > 0 ? Math.max(0, Math.round(((income - expenses) / income) * 100)) : 0 };
  }, [periodTransactions]);
  const dashboardKpis = useMemo(() => {
    const totalBalance = localAccounts.filter((item) => item.icon !== "card").reduce((sum, item) => sum + item.balance, 0) + accountBalanceAdjustment;
    const { income, expenses, savingsRate } = periodTotals;
    return kpiIcons.map((item) => ({
      ...item,
      value: item.label === "Saldo total" ? formatBRL(totalBalance) : item.label === "Receitas do mês" ? formatBRL(income) : item.label === "Despesas do mês" ? formatBRL(expenses) : `${savingsRate}%`,
      delta: "0%",
    }));
  }, [accountBalanceAdjustment, localAccounts, periodTotals]);
  ...
  const handleVehicleAction = (label: string) => action("Veículo", `${label} registrado na sua conta.`);
  const handleProfileSave = (nextProfile: ProfileData) => {
    ...
    if (usernameChanged && demoP2PContacts.some((contact) => contact.username.slice(1).toLowerCase() === username)) {
      action("Identificador indisponível", "Esse @usuário já está reservado no diretório público.");
      return;
    }
  ...
  const handleDangerAction = (dangerAction: DangerAction) => {
    if (dangerAction === "remove-duplicates") {
      ...
      action("Duplicados revisados", "Lançamentos repetidos foram removidos da sua conta.");
      return;
    }
    ...
    setLocalAccounts([]);
    setLocalCreditCards([]);
    setLocalVehicle(emptyVehicle);
    setLocalCategories([]);
    setPaidBills([]);
    setP2PRequests([]);
    setP2PActivities([]);
    setAccountBalanceAdjustment(0);
    action("Espaço limpo", "O dashboard voltou ao estado inicial sem dados fictícios.");
  };
  ...
  const handleAddAccount = (payload: NewAccountPayload) => {
    const name = payload.name.trim();
    const balance = Number.isFinite(payload.balance) ? payload.balance : 0;
    const duplicate = localAccounts.some((item) => item.name.trim().toLocaleLowerCase("pt-BR") === name.toLocaleLowerCase("pt-BR"));
    if (duplicate) {
      action("Conta já cadastrada", "Escolha outro nome para identificar esta conta.");
      return;
    }
    const accountTones: Account["tone"][] = ["mint", "lavender", "peach", "blue"];
    const nextAccount: Account = { name, number: "", value: formatBRL(balance), balance, change: "Conta cadastrada", tone: accountTones[localAccounts.length % accountTones.length], icon: payload.icon };
    const nextAccounts = [...localAccounts, nextAccount];
    setLocalAccounts(nextAccounts);
    void persistNow({ localAccounts: nextAccounts }).catch(() => undefined);
    setSelectedAccount(name);
    setActionPanel("accounts");
    action("Conta adicionada", `${name} · saldo inicial de ${formatBRL(balance)}.`);
  };
  ...
  const handleSupportSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSupportView("home");
    closePanels();
    action("Mensagem enviada", "O atendimento foi registrado com sucesso.");
  };
  const handleLogout = () => {
    void logout();
  };
  const handleSetupComplete = (data: InitialSetupData) => {
    setProfile(data.profile);
    setLocalAccounts(data.accounts);
    setLocalCreditCards(data.cards);
    setLocalTransactions([]);
    setLocalVehicle(emptyVehicle);
    setLocalCategories([]);
    setPaidBills([]);
    ...
    action("Espaço configurado", "Seus dados começam zerados e serão salvos na sua conta.");
  };
  const handleLogin = () => {
  ...
  if (!onboardingComplete) return <InitialSetupScreen email={profile.email || user?.email || ""} onComplete={handleSetupComplete} />;
    ...
    <div className={`app-shell ${compactMode ? "app-shell--compact" : ""} ${hideValues ? "app-shell--values-hidden" : ""}`}>
      ...
      <main className={`main-area ${sidebarCollapsed ? "main-area--wide" : ""}`}>
        ...
        <motion.div className="dashboard-content" initial="hidden" animate="visible" variants={pageVariants}>
          ...
          <motion.section className="dashboard-grid dashboard-grid--secondary" variants={stagger}><motion.article id="spending" className="surface-card spending-card" variants={child}><CardHeader eyebrow="GASTOS" title="Por categoria" subtitle="Distribuição" action={<button className="text-button" onClick={() => setActionPanel("report")}>Relatório <ChevronRight size={14} /></button>} /><div className="spending-content"><div className="donut-wrap"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={spendingData} dataKey="value" nameKey="name" innerRadius={62} outerRadius={87} paddingAngle={3} stroke="none">{spendingData.map((entry) => <Cell key={entry.name} fill={entry.color} />)}</Pie><Tooltip formatter={(value) => formatBRL(Number(value))} contentStyle={{ border: 0, borderRadius: 12, boxShadow: "0 10px 30px rgba(22, 32, 54, .12)", fontSize: 12 }} /></PieChart></ResponsiveContainer><div className="donut-center"><strong>{formatCompactBRL(spendingData.reduce((sum, item) => sum + item.value, 0))}</strong><span>gastos</span></div></div><div className="spending-list">{spendingData.length ? spendingData.map((item) => <div className="spending-row" key={item.name}><span><i style={{ background: item.color }} />{item.name}</span><strong>{formatCompactBRL(item.value)}</strong><small>{Math.round((item.value / spendingData.reduce((sum, entry) => sum + entry.value, 0)) * 100)}%</small></div>) : <div className="table-empty"><FileText size={18} /><strong>Sem despesas</strong><span>Adicione um lançamento.</span></div>}</div></div></motion.article>

            <motion.article className="surface-card accounts-card" variants={child}><CardHeader eyebrow="CONTAS" title="Suas contas" subtitle="Saldo por conta" action={<button className="text-button" onClick={() => setActionPanel("accounts")}>Gerenciar <ChevronRight size={14} /></button>} /><div className="account-list">{localAccounts.filter((account) => account.icon !== "card").length ? localAccounts.filter((account) => account.icon !== "card").map((account) => <button className="account-row" key={account.name} onClick={() => { setSelectedAccount(account.name); setActionPanel("accounts"); }}><span className={`account-icon account-icon--${account.tone}`}>{navIcon(account.icon)}</span><span className="account-info"><strong>{account.name}</strong><small>Conta bancária</small></span><span className="account-value"><strong>{account.value}</strong><small className={account.tone === "peach" ? "account-alert" : ""}>{account.change}</small></span><ChevronRight className="account-chevron" size={15} /></button>) : <div className="table-empty"><Wallet size={18} /><strong>Sem contas</strong><span>Cadastre a primeira.</span></div>}</div></motion.article></motion.section>

          <motion.section className="dashboard-grid dashboard-grid--secondary" variants={stagger}><motion.article id="budget" className="surface-card budget-card" variants={child}><CardHeader eyebrow="ORÇAMENTO" title="Uso dos envelopes" subtitle="Limites do mês" action={<button className="text-button" onClick={() => setActionPanel("budget")}>Ajustar <ChevronRight size={14} /></button>} />{budgets.length ? <><div className="budget-list">{budgets.map((item) => <div className="budget-row" key={item.label}><div className="budget-row-head"><span>{item.label}</span><strong>{item.value} <small>/ {item.limit}</small></strong></div><div className="progress-track"><motion.div className={`progress-fill progress-fill--${item.tone}`} initial={{ width: 0 }} whileInView={{ width: `${Math.min(item.progress, 100)}%` }} viewport={{ once: true }} transition={{ duration: 0.75, delay: 0.15 }} /></div></div>)}</div></> : <div className="table-empty"><Wallet size={18} /><strong>Sem orçamento</strong><span>Defina seus limites.</span></div>}</motion.article>

            <motion.article id="bills" className="surface-card bills-card" variants={child}><CardHeader eyebrow="COMPROMISSOS" title="Contas a pagar" subtitle="Agenda financeira" action={<button className="text-button" onClick={() => setActionPanel("schedule")}>Agendar <ChevronRight size={14} /></button>} /><div className="bills-list">{upcomingBills.length ? upcomingBills.map((bill) => <button className={`bill-row ${paidBills.includes(bill.label) ? "is-paid" : ""}`} key={bill.label} onClick={() => { setSelectedBill(bill.label); setActionPanel("pay-bills"); }}><span className={`bill-date bill-date--${bill.tone}`}><CalendarDays size={15} /><small>{bill.date}</small></span><span className="bill-info"><strong>{bill.label}</strong><small>{paidBills.includes(bill.label) ? "pago" : bill.days}</small></span><span className="bill-amount">{paidBills.includes(bill.label) ? "Pago" : bill.amount}</span><ChevronRight size={15} /></button>) : <div className="table-empty"><CalendarDays size={18} /><strong>Sem compromissos</strong><span>Agende uma conta.</span></div>}</div></motion.article></motion.section>

          <motion.section id="transactions" className="surface-card transactions-card" variants={child}><CardHeader eyebrow="ATIVIDADE RECENTE" title="Últimos lançamentos" subtitle="Clique em um lançamento para editar" action={<button className="text-button" onClick={() => { closePanels(); setActiveNav("Extrato"); setCurrentView("statement"); setMobileFabOpen(false); action("Extrato completo aberto", `Todos os lançamentos de ${selectedPeriod.toLowerCase()} estão disponíveis.`); }}>Ver tudo <ChevronRight size={14} /></button>} /><div className="transactions-table-wrap"><table><thead><tr><th>Data</th><th>Descrição</th><th>Categoria</th><th>Conta</th><th className="amount-cell">Valor</th><th /></tr></thead><tbody>{filteredTransactions.map((transaction) => <tr key={`${transaction.date}-${transaction.payee}-${transaction.amount}`} className="transaction-row" tabIndex={0} role="button" onClick={() => editTransaction(transaction)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); editTransaction(transaction); } }}><td className="muted-cell">{transaction.date}</td><td><span className={`transaction-icon transaction-icon--${transaction.type}`}>{transaction.type === "income" ? <ArrowDownLeft size={15} /> : transaction.type === "transfer" ? <ArrowLeftRight size={15} /> : <ArrowUpRight size={15} />}</span><strong>{transaction.payee}</strong></td><td><span className="category-chip">{transaction.category}</span></td><td className="muted-cell">{transaction.account}{transaction.type === "transfer" && transaction.destinationAccount ? ` → ${transaction.destinationAccount}` : ""}</td><td className={`amount-cell ${transaction.type === "income" ? "income-text" : transaction.type === "transfer" ? "muted-cell" : "expense-text"}`}>{transaction.type === "income" ? "+" : transaction.type === "transfer" ? "↔" : "−"}{formatBRL(transaction.amount).replace("R$ ", "R$ ")}</td><td className="transaction-edit-cell" aria-label="Abrir edição"><ChevronRight size={15} /></td></tr>)}</tbody></table>{filteredTransactions.length === 0 && <div className="table-empty"><FileText size={18} /><strong>Sem lançamentos</strong><span>Adicione seu primeiro.</span></div>}</div></motion.section>
...
type ActionPanelsProps = {
  ...
  onDuplicate: () => void;
  onDelete: () => void;
  onAddAccount: (payload: NewAccountPayload) => void;
  onBudgetSave: () => void;
...
function ActionPanels({ vehicleType, cardCount, actionPanel, profile, usernameLockDaysRemaining, selectedBill, selectedAccount, selectedTransaction, selectedCard, accounts, paidBills, transferAmount, transferDestination, depositAmount, scheduleDate, budgetAdjusted, scheduledReminder, supportView, supportMessage, sessionActive, reportTransactions, transactionTypeFilter, transactionCategoryFilter, transactionCategories, categories, compactMode, alertsEnabled, onClose, onOpenPanel, onProfileSave, onCreateCategory, onUpdateCategory, onToggleCategory, onDeleteCategory, onCompactMode, onAlerts, onDangerAction, onOpenAccounts, onOpenCards, onOpenVehicle, onEditAccount, onUpdateAccount, onUpdateCard, onSelectBill, onSelectAccount, onTransferAmount, onTransferDestination, onDepositAmount, onScheduleDate, onSubmitTransfer, onSubmitDeposit, onMarkBillAsPaid, onSchedule, onExport, onDuplicate, onDelete, onAddAccount, onBudgetSave, onSupportChoice, onSupportMessage, onSubmitSupport, onLogin, onTransactionTypeFilter, onTransactionCategoryFilter }: ActionPanelsProps) {
  ...
  if (actionPanel === "accounts") return dialog("MINHAS CONTAS", account ? account.name : "Minhas contas", account ? `${account.change || "Conta bancária"}.` : "Adicione suas contas manualmente e acompanhe saldos e movimentações.", <Wallet size={17} />, <AccountsPanelContent accounts={accounts} account={account} selectedAccount={selectedAccount} reportTransactions={reportTransactions} onSelectAccount={onSelectAccount} onEditAccount={onEditAccount} onAddAccount={() => onOpenPanel("account-new")} />, <><button className="soft-button" type="button" onClick={onClose}>Fechar</button><button className="primary-button" type="button" onClick={() => onOpenPanel("account-new")}><Plus size={15} /> Adicionar conta</button></>);
  if (actionPanel === "account-new") return dialog("MINHAS CONTAS", "Adicionar conta", "Cadastre uma conta bancária ou carteira manualmente. Nenhum acesso bancário será solicitado.", <Wallet size={17} />, <AccountCreationForm onSave={onAddAccount} onCancel={onClose} />);
  if (actionPanel === "account-edit") return dialog("CARTEIRAS", account ? `Editar ${account.name}` : "Editar conta", "Ajuste o nome e o saldo desta carteira.", <Wallet size={17} />, account ? <AccountEditorForm account={account} onSave={onUpdateAccount} onCancel={onClose} /> : <div className="modal-empty-note"><Wallet size={17} /><span>Selecione uma conta antes de editar.</span></div>);
  ...
  if (actionPanel === "budget") return dialog("ORÇAMENTO", "Ajustar envelopes", "Revise a categoria que passou do limite e salve a sua intenção.", <SlidersHorizontal size={17} />, <div className="budget-dialog"><div className="table-empty"><Wallet size={18} /><strong>Nenhum limite configurado</strong><span>Defina categorias e limites para acompanhar seu orçamento.</span></div>{budgetAdjusted && <p className="dialog-hint"><Check size={14} /> Preferência salva.</p>}</div>, <><button className="soft-button" type="button" onClick={onClose}>Cancelar</button><button className="primary-button" type="button" onClick={onBudgetSave}><Check size={15} /> Salvar ajuste</button></>);
  ...
  if (actionPanel === "privacy") return dialog("SEGURANÇA", "Privacidade e dados", "Confira como seus dados financeiros são tratados com segurança.", <ShieldCheck size={17} />, <div className="privacy-copy"><p>Seus lançamentos ficam vinculados à sua conta e são protegidos pelas regras de acesso do Firebase.</p><div className="privacy-badge"><ShieldCheck size={16} /><strong>Dados protegidos por usuário</strong></div></div>, <button className="primary-button" type="button" onClick={onClose}>Entendi</button>);
  if (actionPanel === "insights") return dialog("INSIGHTS", "Seu próximo movimento", "Uma leitura rápida baseada nos dados que você cadastrou.", <Sparkles size={17} />, <div className="insight-card"><div className="insight-card-icon"><TrendingUp size={18} /></div><div><strong>Ainda não há dados suficientes</strong><p>Adicione lançamentos e limites para receber insights personalizados.</p></div></div>, <button className="primary-button" type="button" onClick={() => { onClose(); onOpenPanel("budget"); }}>Revisar orçamento</button>);
  if (actionPanel === "session") return dialog("SESSÃO", "Sessão encerrada", "O estado local foi mantido neste navegador, mas o perfil está desconectado.", <LogOut size={17} />, <div className="privacy-copy"><p>Você pode entrar novamente para continuar usando seu espaço financeiro.</p><div className="privacy-badge"><ShieldCheck size={16} /><strong>Dados locais preservados</strong></div></div>, <button className="primary-button" type="button" onClick={onLogin}><RefreshCw size={15} /> Reabrir sessão</button>);
...
function AccountsPanelContent({ accounts, account, selectedAccount, reportTransactions, onSelectAccount, onEditAccount, onAddAccount }: { accounts: Account[]; account?: Account; selectedAccount: string | null; reportTransactions: Transaction[]; onSelectAccount: (name: string) => void; onEditAccount: (account: Account) => void; onAddAccount: () => void }) {
  const accountTransactions = account ? reportTransactions.filter((item) => item.account === account.name || item.destinationAccount === account.name || item.sourceId === account.name) : [];
  ...
  return <div className="modal-rich-content accounts-panel"><div className="modal-summary-strip"><div><span>Carteiras</span><strong>{accounts.filter((item) => item.icon !== "card").length}</strong></div><div><span>Patrimônio local</span><strong>{formatCompactBRL(accounts.filter((item) => item.icon !== "card").reduce((sum, item) => sum + item.balance, 0))}</strong></div><div><span>Movimentações</span><strong>{accountTransactions.length}</strong></div></div>{account ? <div className="account-detail-card"><div className={`account-detail-icon account-icon--${account.tone}`}>{navIcon(account.icon)}</div><div><p className="eyebrow">CONTA SELECIONADA</p><h3>{account.name}</h3><span>{account.change || "Conta bancária"}</span></div><strong>{formatBRL(account.balance)}</strong><button className="soft-button soft-button--small" type="button" onClick={() => onEditAccount(account)}><Pencil size={14} /> Editar conta</button></div> : <div className="modal-empty-note"><Wallet size={17} /><span>Selecione uma carteira abaixo para ver seus detalhes.</span></div>} {account && <div className="account-flow-strip"><div><span>Entradas no período</span><strong className="income-text">+{formatCompactBRL(inflow)}</strong></div><div><span>Saídas no período</span><strong className="expense-text">−{formatCompactBRL(outflow)}</strong></div></div>}<div className="modal-section-heading"><div><p className="eyebrow">MINHAS CONTAS</p><h3>Onde seu dinheiro está</h3></div><button className="primary-button primary-button--small" type="button" onClick={onAddAccount}><Plus size={14} /> Adicionar conta</button><span className="modal-section-count">{accounts.filter((item) => item.icon !== "card").length} contas</span></div><div className="action-list action-list--rich">{accounts.filter((item) => item.icon !== "card").map((item) => <button key={item.name} className={`action-list-item ${selectedAccount === item.name ? "is-selected" : ""}`} onClick={() => onSelectAccount(item.name)}><span className={`account-icon account-icon--${item.tone}`}>{navIcon(item.icon)}</span><span className="action-list-copy"><strong>{item.name}</strong><small>{formatBRL(item.balance)}</small></span><span className="status-pill">{item.change}</span><ChevronRight size={15} /></button>)}</div></div>;
}

function AccountCreationForm({ onSave, onCancel }: { onSave: (payload: NewAccountPayload) => void; onCancel: () => void }) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const numericBalance = balance.trim() ? parseEditableBRL(balance) : 0;
    if (normalizedName.length < 2) { setError("Informe um nome para identificar a conta."); return; }
    if (!Number.isFinite(numericBalance)) { setError("Informe um saldo inicial válido."); return; }
    onSave({ name: normalizedName, balance: numericBalance, icon });
  };
  return <form className="transaction-form wallet-editor-form" onSubmit={submit}><div className="form-grid"><label className="form-field form-field--wide"><span>Nome da conta</span><input autoFocus value={name} onChange={(event) => { setName(event.target.value); setError(""); }} placeholder="Ex.: Conta principal" /></label><label className="form-field"><span>Saldo inicial</span><div className="amount-shell"><b>R$</b><input inputMode="decimal" value={balance} onChange={(event) => { setBalance(event.target.value); setError(""); }} placeholder="0,00" /></div></label><label className="form-field"><span>Tipo</span><select value={icon} onChange={(event) => setIcon(event.target.value)}><option value="bank">Conta bancária</option><option value="wallet">Carteira</option></select></label></div><p className="dialog-hint"><ShieldCheck size={14} /> O cadastro é manual. Não solicitamos número da conta, senha ou acesso ao banco.</p>{error && <p className="form-error" role="alert">{error}</p>}<div className="modal-actions"><button className="soft-button" type="button" onClick={onCancel}>Cancelar</button><button className="primary-button" type="submit"><Plus size={15} /> Adicionar conta</button></div></form>;
}
...
function AccountEditorForm({ account, onSave, onCancel }: { account: Account; onSave: (account: Account) => void; onCancel: () => void }) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    onSave({ ...account, name: name.trim(), number: "", balance: adjustedBalance, value: formatBRL(adjustedBalance) });
  };
