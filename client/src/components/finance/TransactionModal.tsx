      ...
      {open && (
        <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
          <motion.div className="modal-panel modal-panel--transaction" role="dialog" aria-modal="true" aria-labelledby="transaction-modal-title" initial={{ opacity: 0, y: 18, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.98 }} transition={{ duration: 0.22 }}>
            <div className="modal-header"><div className="modal-heading"><span className="modal-icon"><Repeat2 size={17} /></span><div><p className="eyebrow">{editing ? "EDITAR MOVIMENTAÇÃO" : "MOVIMENTAÇÃO MANUAL"}</p><h2 id="transaction-modal-title">{editing ? "Editar transação" : "Adicionar transação"}</h2><p>{editing ? "Atualize os dados deste lançamento." : "Registre uma entrada, saída ou compra no cartão."}</p></div></div><button className="modal-close" type="button" onClick={close} aria-label="Fechar modal"><X size={18} /></button></div>
            <form className="transaction-form" onSubmit={submit}>
              ...
              {type === "expense" && sourceType === "account" && <div className="transaction-switch-row"><Switch id="transaction-use-card" checked={false} onCheckedChange={(checked) => { if (checked) selectSourceType("credit-card"); }} /><label htmlFor="transaction-use-card"><strong>Lançar no cartão</strong><span>Use a fatura para parcelar ou programar esta despesa.</span></label></div>}
