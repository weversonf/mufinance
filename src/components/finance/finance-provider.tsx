...
function buildData(snapshot: FinanceSnapshot | null): Omit<FinanceDataValue, "snapshot" | "loading" | "error" | "refresh"> {
  ...
  const bankAccounts: BankAccount[] = visibleAccounts.map((account, index) => ({ id: account.id, name: String(account.name ?? "Conta"), type: account.type === "savings" ? "savings" : "checking", institution: "Conta manual", institutionLogo: "/icon.svg", accountNumber: "", balance: numberValue(account.balance), currency: "BRL", change: 0, changePercent: 0, lastActivity: "", color: ["bg-primary", "bg-emerald-500", "bg-violet-500", "bg-orange-500"][index % 4] }))
